...
- **Two surfaces of one product can resolve the same manifest differently.**
  ...
  surface.
- **The same split covers agents, and one field is on the wrong side of it on
  purpose.** Agent Plugins 1.0 allows no configurable paths: its agents live
  at the fixed `com.github.copilot/agents/`, which is where TBaguette's three
  delegation roles sit. The `"agents"` key in `plugin.json` is the legacy
