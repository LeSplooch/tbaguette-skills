...
def check_i18n_document_shell() -> None:
    ...
    check("French index carries an hreflang alternate for every one of the 16 locales, "
          "plus x-default (17 total — x-default's own <link> tag matches this same "
          "counted substring)",
...
(`strings.footer_brand_html_template`, once formatted with the two brand-name constants, is pre-formatted HTML — trust-boundary convention as `body_html` elsewhere, which is why the *result* isn't run through `escape_html`. The two brand names themselves come from `BRAND_BAKERY`/`BRAND_ATELIER` Python literals, never from translatable content — this is the fix for the brand-identity constraint violation the Task 3 reviewer found: the original draft of this field baked both brand names directly into the translatable string.)
...
Run: `cd scripts && python3 test_templates.py`
Expected: all checks pass, including every assertion added in Step 1. `_render_footer` was fully rewired above (not deferred to a later task — its `strings=strings` call site inside `_render_document` would otherwise raise `TypeError` immediately, failing *every* test in the file, not just the new ones), so nothing in this task's own new test should be red. `_render_hero`/`_render_search_empty_state`/`_render_breadcrumb`/`_render_prevnext`/`_render_see_also`/`_render_skill_head` are still called with their old (pre-locale) signatures inside `render_index`/`render_skill_page` at this point — that's fine and deliberate (Task 4 rewires them), since none of *this* task's assertions exercise those inner links, only the document shell (lang/dir/hreflang/canonical/switcher) they wrap. If anything is red, fix it now — do not carry a failing test into Task 4.

- [ ] **Step 8: Run the full suite**

Run: `python3 scripts/run_tests.py`
Expected: `All 6 suites passed, N checks total.` (`test_i18n.py`, added in Task 1, already made it 6 suites — 5 original + `test_i18n.py`; Task 8 later adds locale-build coverage to `generate.py` itself, not this task).
...
(`strings.install_note_restart_html_template`, once formatted, is pre-formatted HTML, injected verbatim — same trust-boundary convention as `footer_brand_html_template`. Both `frame_label` and `restart_note_html` splice in `BRAND_NAME` as a Python literal rather than letting the brand name live inside translatable content — same fix as `_render_footer` above.)
...
- Brand names never appear as literal text in a translated field at all — three fields (`footer_brand_html_template`, `install_frame_label_template`, `install_note_restart_html_template`) carry a `{brand}` (or `{brand_bakery}`/`{brand_atelier}`) placeholder instead, which Python fills in with the real, untranslated name at render time (see `BRAND_NAME`/`BRAND_BAKERY`/`BRAND_ATELIER` in `templates.py`, added as part of Task 3's brand-identity fix). Write your sentence in natural French *around* that placeholder token, in whatever word order French wants — do not substitute the literal English brand name yourself, and do not drop the placeholder.
- Every other `{name}` placeholder (e.g. `{skill_count}`, `{category_count}`, `{category}`, `{language}`) is copied through **verbatim**, unchanged — only its surrounding words move to fit French word order.
- Every `*_html` field (e.g. `footer_brand_html_template`, `install_note_restart_html_template`) keeps its embedded `<code>`/`<strong>` tags exactly where they wrap the same words they wrap in English, translating only the text between/around them.
...
```json
{
  ...
  "install_frame_label_template": "Installer les compétences de {brand}"
}
```
...
> **Scope closed 2026-08-15 at 12 of 14, by explicit user instruction:** "keep the completed languages that we have and call it a day. No more languages to process." Vietnamese, Polish, and Indonesian were never dispatched. They briefly stayed registered in `scripts/locales.py` with no `i18n/<code>/` directory (the fallback design's supported "not translated yet" state — the same one every one of the 12 completed locales was in before its own turn) — but were then removed from the registry entirely at a further explicit instruction the same day ("Remove the unimplemented languages from the language list that's on the website"), since a switcher entry that only ever leads to an all-English page under a foreign-looking URL prefix isn't a good long-term default. `EXPECTED_LOCALE_COUNT` is now `13` (was `16`). Resuming any of the three later means re-adding its `Locale` entry, bumping `EXPECTED_LOCALE_COUNT` back up, then running the per-locale procedure below — the exact reverse of the removal, not a rebuild from scratch. A full digest of every judgment call and deferred item from executing this task is at the end of this document, under "Task 12 execution record."
...
| `de` | German | ✅ done | — |
| `it` | Italian | ✅ done | — |
| `pt` | Portuguese | ✅ done | Brazilian Portuguese conventions (matches `hreflang="pt-BR"` in the locale registry) |
| `ru` | Russian | ✅ done | — |
| `zh` | Chinese | ✅ done | Simplified characters (matches `hreflang="zh-Hans"`) |
| `ja` | Japanese | ✅ done | — |
| `ko` | Korean | ✅ done | — |
| `hi` | Hindi | ✅ done | — |
| `ar` | Arabic | ✅ done | RTL — the page mirrors automatically via `dir="rtl"` (Tasks 3, 9); nothing about writing the JSON content itself changes, translate the prose normally |
| `tr` | Turkish | ✅ done | — |
| `vi` | Vietnamese | ❌ removed from registry | Not started; no longer in `scripts/locales.py` as of 2026-08-15 |
| `pl` | Polish | ❌ removed from registry | Not started; no longer in `scripts/locales.py` as of 2026-08-15 |
| `id` | Indonesian | ❌ removed from registry | Not started; no longer in `scripts/locales.py` as of 2026-08-15 |
...
**Actual outcome:** 12 of 14 done (see the scope-closure note above the table). `vi`/`pl`/`id` no longer appear in `i18n_status.py`'s report at all — they're not registered locales anymore, not just unpopulated ones. Whenever this resumes, re-add the locale to `scripts/locales.py` and bump `EXPECTED_LOCALE_COUNT`, then the per-locale procedure above is unchanged — pick up at Step 1 for that row.

---

## Task 12 execution record

A distilled record of the judgment calls and deferred items from executing Task 12, before the working ledger (`.superpowers/sdd/2026-08-14-website-i18n/progress.md`, git-ignored scratch) is cleaned up. Full detail — every reviewer's reasoning, every codepoint check, every self-caught translation error — lives in that ledger for as long as it survives; this section is the part meant to outlive it.

### Cross-cutting infrastructure bugs found and fixed along the way

Real translated content in each new locale repeatedly surfaced latent bugs in already-reviewed shared code — bugs synthetic test fixtures never triggered because they never resembled the real thing closely enough. All were fixed, reviewed, and merged before the triggering locale's own review completed:

1. **`category_title` never reached per-skill cards/breadcrumbs/tags** (found via French) — only the top-level category list got the translation. Fixed alongside a latent multi-word-heading-id bug in the same commit.
2. **Homepage "See how" link ignored locale** (found via Spanish) — always pointed at the English-root verify-install page.
3. **Search empty-state quote marks were hardcoded English curly quotes** in `site.js` (found via German) — the one remaining un-internationalized string in that file.
4. **`summarize_description()` and two hardcoded sentence-terminal periods were ASCII/CJK-punctuation-blind** (found via Chinese) — card-teaser truncation and two template strings both silently assumed Latin punctuation conventions.
5. **Four distinct RTL/bidi rendering bugs plus an Arabic-punctuation truncation gap** (found via Arabic, the only RTL locale) — the untranslated-English-fallback body inherited `dir="rtl"` and visually scrambled; the homepage install command and all inline `<code>` spans lacked bidi isolation; prev/next arrow glyphs aren't Unicode `Bidi_Mirrored` and pointed the wrong way; and Arabic's dedicated `،`/`؛` punctuation wasn't recognized by the truncation logic. This was the single largest fix of the whole task — RTL support had been unit-tested for months but never visually exercised by real RTL content until Arabic's batch.

### Deliberately deferred, not fixed

- **A shared 2-slot singular/plural schema doesn't cover every real language's plural system.** Russian (3-way numeral agreement), Arabic (6 CLDR plural categories), and Korean's numeral-counter spacing convention all hit variations of this. Each locale's chosen pair is correct for all-but-at-most-one of the real category counts in the current corpus (Arabic's `12 مهارات` badge is the one confirmed-wrong render today). A proper fix needs a richer per-locale pluralization schema, not a one-off patch — worth doing once, for whichever locales need it, rather than repeatedly.
- **Hindi's danda (`।`) sentence-terminal punctuation** isn't recognized by `summarize_description()`. Empirically confirmed zero current impact (no real Hindi first-sentence is short enough to hit the affected code path) — matters only for future skill-body (Phase 2) content, which is separately out of scope already.
- Roughly 90 locale-specific Minor findings across the 12 batches (phrasing preferences, one-off terminology nits, a few cross-referenced consistency suggestions) — none meaning-changing, all logged with their reviewer's reasoning in the ledger, none blocking.

### Judgment calls the plan's own text got wrong, corrected during execution

- Task 3 and Task 5 originally baked brand identity (`La Boulangerie TBaguette`, `TBaguette's Atelier`) directly into translatable string fields, violating the plan's own constraint that brand identity stay a Python literal. Fixed by introducing `{brand}`/`{brand_bakery}`/`{brand_atelier}` placeholders and `_template`-suffixed field names before any locale's content was written against the old shape.
- The hreflang-tag-count test in Tasks 3 and 8 undercounted by one (didn't account for `x-default`'s own tag matching the same substring it was counting).
- Task 5's `dir="ltr"` regression test asserted `count >= 1`, which the page's own unrelated `<html dir="ltr">` tag trivially satisfied regardless of whether the feature under test worked at all.
- Task 9's two new `[dir="rtl"]` CSS regression checks verified only that a selector existed, never the actual property values — a real regression could have passed undetected.

### Post-close content-drift backfill (two waves, all 12 locales, 2026-08-15)

After the scope closure above, the branch sat on real repo history for several days while other work landed on `master`, and two content gaps opened up independently of anything in this plan:

**Wave 1 — 8 new skills shipped with no translated description in any locale.** Commits landing on `master` after each locale's own batch shipped had added 8 new skill files (`using-tbaguette`, `grounding-test-doubles`, `judging-duplication`, `recovering-agent-context`, `redacting-sensitive-output`, `revalidating-decisions`, `tracking-data-provenance`, `validating-numeric-input`) plus one new clause in the existing `designing-apis` description — none of it translated anywhere, so all 12 locales silently fell back to English body text for those 8 pages. First surfaced by French's implementer correcting an initial, too-narrow dispatch premise ("French is missing 8 descriptions, not 1 — and it isn't French-specific, all 12 locales are missing exactly the same 8"), confirmed by a `grep -L` sweep across all locales, then backfilled locale-by-locale.

**Wave 2 — 6 existing skills' English descriptions had drifted after their translations shipped.** `designing-apis` was already known from wave 1. Turkish's implementer found a 6th (`refactoring-safely`) while doing unrelated work, which prompted a bounded audit (`git diff <merge-base>..HEAD -- 'skills/*/SKILL.md'`, diffed per-file on the `description:` field only) rather than a per-locale patch. That surfaced the full set: `atomic-commits` (two clauses), `choosing-test-scope`, `code-archaeology`, `refactoring-safely` (two clauses), `reproducing-bugs` (two clauses) — all clauses that existed in the real `SKILL.md` files but never made it into any locale's `descriptions.json`. Backfilled across all 12 locales in the same dispatch order as the original batches (fr, es, de, it, pt, ru, zh, ja, ko, hi, ar, tr), each locale reusing its own already-established vocabulary for recurring concepts (AI agent, fixture recording, documented decision, test harness, load-bearing duplication) rather than inventing new terms — confirmed via `git log` afterward that no further drift exists past the point this branch merged from `master`.

Both waves used a lighter process than Task 12's original batches: no separate reviewer dispatch, since the earlier full-locale work had already established each language's quality bar and reference conventions — self-review against a named wrong-referent risk list (the recurring error class throughout Task 12) substituted for a second pass, appropriate to the smaller, well-scoped size of a clause insertion versus a novel sentence.

The branch was then merged forward with five intervening `master` commits (the "fresh from the oven" coverflow's click-and-drag-to-spin, momentum, and a `draggable="false"` Firefox fix) — a clean, non-conflicting merge on every real source file, with the ~990 generated `docs/` conflicts resolved by regenerating rather than hand-merging — and fast-forward pushed to `origin/master`.
