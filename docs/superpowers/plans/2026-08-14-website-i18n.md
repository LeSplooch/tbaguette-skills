...
> **Scope closed 2026-08-15 at 12 of 14, by explicit user instruction:** "keep the completed languages that we have and call it a day. No more languages to process." Vietnamese, Polish, and Indonesian were never dispatched and remain unpopulated — deferred, not abandoned. They stay registered in `scripts/locales.py` (`LOCALES`, `EXPECTED_LOCALE_COUNT=16`) with no `i18n/<code>/` directory, which the fallback design already treats as a supported state (silent full-English fallback, chrome and body alike — the same state every one of the 12 completed locales was in before its own turn). Resuming later is just re-running the per-locale procedure below for `vi`/`pl`/`id`. A full digest of every judgment call and deferred item from executing this task is at the end of this document, under "Task 12 execution record."
...
| `tr` | Turkish | ✅ done | — |
| `vi` | Vietnamese | ⏸ deferred | Not started |
| `pl` | Polish | ⏸ deferred | Not started |
| `id` | Indonesian | ⏸ deferred | Not started |
...
**Actual outcome:** 12 of 14 done (see the scope-closure note above the table). `vi`/`pl`/`id` remain at `--/--/--/--` in `i18n_status.py`. Whenever this resumes, the per-locale procedure above is unchanged — just pick up at Step 1 for the next row.
