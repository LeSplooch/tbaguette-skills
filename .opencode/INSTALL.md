...
```bash
# Remove old symlinks
rm -f ~/.config/opencode/plugins/tbaguette.js
rm -rf ~/.config/opencode/skills/tbaguette

# Optionally remove the cloned repo
rm -rf ~/.config/opencode/tbaguette

# Remove skills.paths from opencode.json if you added one for TBaguette
```
...
1. Check logs: `opencode run --print-logs "hello" 2>&1 | grep -i tbaguette`
...
```powershell
npm install tbaguette-skills@git+https://github.com/LeSplooch/tbaguette-skills.git --prefix "$HOME\.config\opencode"
```
...
```json
{
  "plugin": ["~/.config/opencode/node_modules/tbaguette-skills"]
}
```
...
- Full documentation: https://github.com/LeSplooch/tbaguette-skills/blob/master/README.opencode.md
