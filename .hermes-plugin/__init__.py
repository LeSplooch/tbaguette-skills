...
def _skills_dir() -> str:
    ...
    - git-clone install (`hermes plugins install LeSplooch/tbaguette-skills`):
...
def _build_bootstrap(skills_dir: str) -> str:
    ...
    return (
        ...
        f"The using-tbaguette skill content is included below and is already "
        f"loaded for this Hermes session. Follow it now. "
        f"Do not try to load using-tbaguette again.\n\n"
        f"{body}\n\n"
        f"## Loading TBaguette Skills on Hermes\n\n"
        f"TBaguette skills are registered with Hermes' native skill loader: "
        f'invoke one with `skill_view("TBaguette:skill-name")` '
        f'(for example `skill_view("TBaguette:naming-things")`). '
