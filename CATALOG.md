...
| `clairvoyance` | The one skill that questions the request rather than serving it — and now anything else that arrives with a frame: a plan, a decision, a number, a document, a project, the run itself. Eight directions and the tells that pick one, seven mechanical moves, and the routing that turns sight into a gate rather than into scope. Multi-file: the directions in full, the objects it can sweep, a catalogue of lenses borrowed from other disciplines with the seats they are looked from, `orbit` for the whole project from above, a seat in each of the seven tracks, and the sweep run on its own creation and on its own enlargement |
