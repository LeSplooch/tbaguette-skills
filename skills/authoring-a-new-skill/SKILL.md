...
- Link every reference file directly from `SKILL.md`, so none of them is reachable *only* by passing through another one. An agent previewing a long file under time pressure will sometimes skim rather than read it whole, and a reference reachable only through another reference is one that skimming never reaches. Cross-links between two files that are each already linked from `SKILL.md` are fine — `diagnosing-before-fixing`'s reference files point at each other, and every one of them is still one hop from the skill.
...
The core loop mirrors `writing-the-failing-test-first`, aimed at a paragraph instead of a function: watch the failure, write the minimal fix, close the gap, repeat.
...
One check comes before RED: whether the scenario can reach the claimed failure at all. A single fresh dispatch has no long context to decay, no accumulated turns to drift across — so a skill whose trigger lives in that missing length can come back with a clean baseline every time while the failure it targets stays real. Stacking more pressure into the prompt doesn't close that gap; pressure is not context length. A clean RED there is neither a discharged testing obligation nor evidence the skill is unnecessary. Record it honestly — "did not reproduce, and here is the limit of the scenario" — and ship the skill as guidance rather than as a demonstrated fix. Only a clean RED from a scenario that could have produced the failure argues against shipping.
...
| A clean RED on a long-context failure reported as "tested," or as proof no skill is needed | The scenario never reached the failure — context decay and cross-turn drift cannot occur inside a fresh one-shot dispatch |
