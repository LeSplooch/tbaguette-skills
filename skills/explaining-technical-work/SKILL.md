...
Everything above assumes a document: composed once, revisable before anyone sees it, read by someone who can reply. A progress pane, an activity log, a status line, a streamed step message is none of those, and every writing test above still applies to each line of it.

`formidable`, and its CLI-output reference in particular, owns the surface — which display a wait earns, what goes to stderr, and the verbosity ladder deciding how much detail each level carries. This is the other half: what the line *says* once that ladder has settled how much of it there is. Three things the sections above do not reach, each because the reader is watching rather than reading.

- **Translate at the emission site, because nothing downstream can.** A report is written where the meaning lives; a line is emitted where only the internals do — the stage name, the function, a model's own commentary on its own uncertainty. The ladder answers *how much*, never *in whose vocabulary*, so a default level can be correctly brief and still be written in the system's private language. In one generator the fix was to stop streaming the reasoning trace and send one short line naming what each step had produced; in the same system, the name of the thing being built was computed early and never sent, so the headline showed the reader their own request back until the work finished. One incident each, so treat them as illustrations rather than as a rate — but the shape is the one to expect, because what is nearest to hand at the emission site is what ships and the translation has no later place to happen.
- **Emit on the reader's clock, not the work's.** This is the one with no counterpart above, because a document's reader cannot act halfway through it. A live reader can, so the same true sentence is a different and worse message after they have committed than before. A validator registered and never called since its pane was built reported a bad value on save; the operator had typed through several more fields first and then had to work out which one. Information that exists before the reader's point of no return and arrives after it is not late — it is a different defect, and the fix is a position in the flow rather than a rewording.
- **Silence is a claim, and it is the one claim you never wrote.** From outside, a correct quiet run and a hang are one observation. `formidable` sets what a wait owes the reader and `automating-repetition` covers the heartbeat that tells a working job from a dead one; the writing half is only that going quiet asserts *nothing is happening* to someone who cannot check, so whatever you would rather they not conclude has to be said instead of left to inference.
