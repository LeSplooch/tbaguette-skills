...
- Mandatory before merging to main, after completing a major feature, and after each task when the work is being driven via `subagent-driven-development`.
...
- Not yet: your own checks haven't passed — tests, lint, a manual run (see `verification-before-completion`). A reviewer finding what the test suite would have found is attention spent on something that was still yours to catch.
...
- Not for: how to actually perform a review — that's the reviewer's job (see `reviewing-code-deeply`). Once the review comes back, acting on it — fixing, pushing back, deciding what's worth deferring — is `receiving-code-review`, the other half of this pair; this skill ends at the request.
...
- **Scope, stated explicitly.** A subagent reviewer works read-only — inspect with `git show` / `git diff` / `git log`, never mutate the working tree, index, HEAD, or branch state; a different revision to inspect means a separate worktree (see `using-git-worktrees`), never a checkout on top of yours. And it reviews the range itself: no spawning a second reviewer for part of the diff. One review seat, spent directly, in passes if the diff demands it.
