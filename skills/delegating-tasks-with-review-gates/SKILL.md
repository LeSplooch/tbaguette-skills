...
Keep a running written record outside your own context, updated after every task and every fix round: which tasks are closed, which commits closed them, and every ruling made along the way. If this loop is one phase of a larger run, that record already exists and has a shape — `orchestrating-work-end-to-end` defines it, and a second file competing with it is two accounts of progress that will disagree. Tick the plan's own `- [ ]` checkboxes as tasks close, too — a plan written by `structuring-an-implementation-plan` carries them for exactly this, and letting them drift out of step with your record leaves two accounts of progress that disagree. Compaction erases what you remember; it does not erase that file or `git log`. Recovering from a lost context means trusting the record and the log over your own recollection — a controller that trusts its memory instead re-dispatches work that's already done, which is the single most expensive mistake this loop can make.
