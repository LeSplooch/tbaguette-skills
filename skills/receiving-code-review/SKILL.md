...
- Not for: producing the review in the first place (see `reviewing-code-deeply`, `requesting-code-review`). A self-review pass that comes back clean is a different risk, owned by `karen-and-the-manager` — that skill exists because your own satisfied review is suspect; this one exists because someone else's feedback earns the same scrutiny, not automatic agreement.
