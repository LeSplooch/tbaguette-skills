...
- **Never read what you already have.** A file read at turn 3 is still in context at turn 30. In that same sample this went wrong in 44% of sessions, so it is worth an explicit check rather than trust. Re-reading it does not refresh anything; it buys a second copy at full price. This includes re-reading a file you just edited to confirm the edit — the edit already reported success or failed loudly.
...
|---|---|
| Adding a tool, plugin, or MCP server to save tokens | Its schema joins the per-request floor and is re-sent every turn, so it is charged whether or not it is used. To break even it has to save more than it costs on every run, and the bounded-read tools most such servers offer — ranged read, grep, glob — are usually already in the harness. Measure the schema against the saving before adding one |
