...
- **A guard written as a comparison is not a parse.** A relational check is a denylist over an ordered domain, and floating-point NaN is the member nobody enumerated — it makes every comparison false, so a pair of bounds that looks exhaustive lets it through untouched. Assert the value *is* a finite number before asking anything about its size. `validating-numeric-input` covers the rest of that domain.
...
**Before the model, the harness parses.** Text sent in as a *prompt* to an agent — rather than returned as a tool result — often passes through a grammar first: a leading slash dispatches a command, an at-sign pulls in a file, a template expands. Unlike the model, that layer has a grammar, so it has a separating mechanism too. Forwarding someone else's text — an issue title, a chat message, a webhook payload — into another agent's prompt without switching that grammar off lets the text pull in files or trigger commands before any model has weighed it. Use the host's literal or verbatim prompt mode where it offers one; where it does not, deliver the text as an attachment or a tool result instead of as prompt text.
...
**Invisible text is the denylist that most often comes up short.** A sanitizer that strips "zero-width and bidi characters" before untrusted text reaches a model or a reviewer has listed the two families everyone remembers and let the rest through: the tag block `U+E0000`–`U+E007F`, which mirrors ASCII invisibly; the invisible operators `U+2060`–`U+2064`; the soft hyphen `U+00AD`; the Mongolian vowel separator `U+180E`. Each reaches a model intact and renders blank for the person approving the text. Where the text only has to be read, allowlist what it needs — letters, marks, digits, punctuation, symbols, spaces, and the controls you mean — and drop the format (`Cf`) and private-use categories rather than growing the list, adding back by name the few you need, such as the joiner inside emoji sequences. Variation selectors are classed as marks, so a category rule keeps them and they can still carry hidden bytes; decide those explicitly. Test the scrubber with a tag-block string, not with a zero-width space. `auditing-dependencies` covers the same characters where the carrier is a dependency's own prose.

Where you do filter, **the replacement must not quote the match** — a redactor that embeds the first N characters of what it matched, for debuggability, reproduces the thing it exists to remove and hands it to the consumer the filter was protecting. Test by asserting the **absence of the input**, never the presence of the marker, which is trivially satisfiable while the removal fails. Applying this across logs, traces, and crash dumps is `redacting-sensitive-output`.
