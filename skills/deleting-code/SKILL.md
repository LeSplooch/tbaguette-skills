...
Then instrument. Add a counter or a sampled log at the entry of the suspected-dead path, ship it, and wait longer than the longest business cycle that could reach it — 30 days as an absolute floor, and one full billing or fiscal cycle for anything customer-facing. Delete when the counter is zero **and you can explain why it is zero**. Zero from a build that never reached two regions is not evidence; confirm the instrumented version is actually running everywhere before you start the clock.
