...
- **States** all present: hover or focus, active, disabled, loading, empty, error, selected, stale, and whatever the stack adds.
- **Stale ages from the data, not from the connection.** A live readout has two clocks — whether the connection is still open, and the age of the newest datum — and only the second is honest, because the connection is built to be tolerant and the person is not: a green dot and a latency figure outlive a dead source for as long as the idle timeout. Derive freshness from the last real update, show the age once it passes the cadence the source promises, and verify by killing the source and watching the indicator turn within one cadence.
- **A derived value comes from one snapshot.** Any readout computed from more than one field is computed from fields sampled together, or it says which input is behind; a ratio whose numerator and denominator are written on different cadences displays a state the system was never in. A displayed time is rendered from a full instant — date, time, zone — never from a bare wall-clock string that a parser rejects and the surface then shows raw.
...
- **Motion shorter than the capture's latency is verified from frames, not from a still.** A still has a capture latency of its own — the time from asking for it to the frame that comes back — and any change that completes inside that window is invisible to it by construction: a quarter-second capture path returns the resting state of a quarter-second animation every time, and on [motion.md](motion.md)'s own table every micro state is shorter than that. Record the moment instead, split the recording into frames at a rate that puts several inside the shortest animation under test, and take one number per frame from the region that should change — a brightness, an edge position. "Looks right" becomes a curve checked against the spec: starts within a frame of its trigger, holds as long as its cause lasts, resolves over the stated duration. Write the capture latency beside the animation's duration before choosing the instrument: where the first exceeds the second, a still that shows nothing has measured the resting state, not the motion — the UI form of `red-teaming-your-own-work`'s *A null result is bounded by what the search could have seen*.
