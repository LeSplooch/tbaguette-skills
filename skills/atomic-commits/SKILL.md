...
- Not for: what the message should say — see `writing-commit-messages`. Not for choosing merge, squash, or rebase at integration time — see `landing-a-finished-branch`. Not for deciding whether a restructure preserves behavior — see `refactoring-safely`; this skill only covers keeping that restructure in its own commit once the decision is made.
