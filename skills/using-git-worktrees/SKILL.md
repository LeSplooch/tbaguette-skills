...
- Before running a multi-step plan, alongside `executing-plans` — a bad step partway through shouldn't cost you the checkout you started from.
- Several independent tasks about to run against the same repo at once, pairing with `dispatching-parallel-agents` — each one needs a workspace nothing else is writing to concurrently.
...
- Not for: deciding whether to merge, rebase, or discard a finished branch (see `finishing-a-development-branch`).
