...
Write the failing test first — the smallest reproduction, automated if a framework is available, a one-off script if it isn't. A fix nobody watched fail against a red test is running on faith; `test-driven-development` covers writing it properly and `verification-before-completion` covers confirming the result before calling it done.
