...
- A plan exists — typically written by `writing-plans` — and it's ready to execute task by task in the current session.
...
- Not for: two or more tasks that are genuinely independent of each other and can run concurrently, with nothing gating one on another's result (see `dispatching-parallel-agents`).
- Not for: running a plan's tasks inline in the current session's main thread, with no fresh subagent per task (see `executing-plans`).

## Before task one

Do this work in an isolated workspace — a worktree or a feature branch, never a shared trunk directly (see `using-git-worktrees` for setting one up).
...
Keep a running written record outside your own context, updated after every task and every fix round: which tasks are closed, which commits closed them, and every ruling made along the way. Tick the plan's own `- [ ]` checkboxes as tasks close, too — a plan written by `writing-plans` carries them for exactly this, and letting them drift out of step with your record leaves two accounts of progress that disagree. Compaction erases what you remember; it does not erase that file or `git log`. Recovering from a lost context means trusting the record and the log over your own recollection — a controller that trusts its memory instead re-dispatches work that's already done, which is the single most expensive mistake this loop can make.
...
Once every task is done, dispatch one review across the whole branch — not per-task scope this time, and on the most capable model available regardless of what individual tasks used. Compose that dispatch the way `requesting-code-review` describes: what was built, what it should do, the exact range, and the shape you want the findings back in. Hand it the running list of deferred minors and parked findings too, so it triages what actually blocks merge instead of rediscovering all of it from nothing.
...
Once the final review is clean, this skill's job is done. How the branch actually lands — merge, pull request, or something else — belongs to `finishing-a-development-branch`, not here.
