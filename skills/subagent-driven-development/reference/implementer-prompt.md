...
Dispatch a fresh subagent:
  ...
  model: [MODEL — REQUIRED: choose per SKILL.md's "Choosing a model for each role" table. An
         unspecified model silently inherits the session's own, usually the
...
- `[MODEL]` — REQUIRED: implementer model per SKILL.md's "Choosing a model for each role" table
- `[task name]` — this task's name from the plan
- `[BRIEF_FILE]` — the file holding this task's full requirements, extracted from the plan
- `[Scene-setting...]` — one line on where this task fits, plus any interfaces or decisions from earlier tasks the brief itself has no way to know about
- `[directory]` — where the implementer works from
- `[REPORT_FILE]` — the file the implementer writes its report to; name it so it's obviously paired with the brief (for example, a task's brief and report share a number)

## What the implementer returns

A short status contract (DONE, DONE_WITH_CONCERNS, BLOCKED, or
NEEDS_CONTEXT), the commits it created, a one-line test summary, any
concerns, and the report file path — never the full report inline.
