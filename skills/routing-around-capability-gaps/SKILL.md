...
**A call carrying several effects cannot report its own refusal, and that is a defect you introduced before it was refused.** The stop rule itself is unaffected by this — you stop on a refusal either way, and knowing which effect was withheld changes nothing about that. What it costs is the *report*, which is the only useful thing a blocked run produces. One line chaining a search, an edit, and a network fetch is refused as a unit, and the message names none of the three, so the report cannot say what is blocked and a human has to re-derive it. Do not go finding out by probing: each narrower attempt is another reach at an effect that may never have been permitted, and by the time the answer arrives the call has been made. Name the refused call whole, exactly as it was issued, rather than a guess at which part of it was the problem.
