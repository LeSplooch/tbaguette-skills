...
**When a change is named after a class of defect, hunt that class inside the change.** Work that removes silent failures, unbounded waits, or a false statement is the last place anyone looks for a fresh instance of the same class. Its author has just built the mental model that says the class is gone, and that model is what makes the new instance unreadable. So take the class the change claims to remove and search the diff for it by name, as though someone else had written the diff and made the same claim. Two instances: a startup path cleared of silent failures that gained a wait-forever call before anything was listening, and a bound placed on the outer loop of a walk, so that it limited the shape of the tree rather than the work. When the fix replaces a claim (an error message, a status line, a sentence in a report), ask what the *new* claim is false about, and list every branch that reaches it. A replacement that names one reason for a missing value, when two different reasons reach that branch, is the original defect moved one layer in.
