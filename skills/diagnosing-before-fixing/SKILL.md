...
The same reflex misfires in the other direction, and there it produces a wrong report rather than a stopped search. When the thing has only ever been *seen* through something local — a preview server, a viewer, an emulator, a staging copy — the environment is not the lazy hypothesis to be resisted. It is the first one, because it sits between you and the object and it is the part that is not the deliverable. A local server that sends no charset leaves the client to guess one, and every em dash of a perfectly valid document comes back as mojibake; a base path, a compression setting, a stale cache, a viewer substituting a font it does not have, a terminal's own encoding each do the same for their own class of defect. All of them look like faults in the artifact, because presentation is precisely what they change. Reported as seen, that is a defect filed against something that does not have it — and then fixed there, in an artifact that was already correct. The remedy is this section's own, pointed the other way: **before writing down anything observed only through a local stand-in, obtain the same artifact through the path its real consumers use, and compare.** A difference between the two is a fact about the stand-in, measured rather than attributed. `confirming-before-claiming-done`'s *Evidence does not travel to another environment* makes the matching argument for a green result; a red one inherits that limit unchanged.

Comparing against the real consumers' path sorts faults of presentation, and it can mis-sort faults of execution. Where the stand-in differs by *not running* something, or running it late — a hidden browser pane that gets no animation frames or scroll events, an app the operating system has suspended, a host that sleeps — the real path runs it and passes, so every failure seen only in the stand-in looks like the stand-in's, including the ones that are not. One question sorts them: **can production be in the stand-in's state?** A scroll position never saved because the test's script, standing in for a user, scrolled a hidden pane and no scroll event fired belongs to the stand-in: no user scrolls a view they cannot see. A restore queued for the next animation frame belongs to the code: users leave views in the background too, and there it waits exactly as long as it did in the test. Fix that one by removing the dependence on what the host withheld, not by bringing the test to the foreground. `finishing-what-you-started` covers what the same kind of host does to a pass: a clean zero from an instrument that never ran.

None of that softens the main rule, and none of it contradicts the section below. An environmental cause really is a legitimate place for an investigation to end, and documenting one honestly is the right outcome when it is true. Documenting it is simply not the same act as establishing it, and the gap between the two is usually one command that takes under a second. Nor is this `bounding-autonomous-work`'s cheapest experiment wearing different clothes: that one tests a claim about your own capabilities before handing work back to a person, this one tests a claim about the world before you stop looking. The tell is the same either way — a conclusion resting on something you never contacted.
...
| A check that fails only in a hidden, suspended, or sleeping host is put down to the host | Nobody asked whether production is ever in that state; where it is, the host exposed the defect rather than causing it |
