...
**A flag, not config,** when the value must change without a restart, targets a subset of users, or gets flipped during an incident. `feature-flagging` owns what happens next — the four flag types, each with its own lifespan, and the expiry that keeps a release flag from becoming permanent. Config is deploy-scoped and fleet-wide by nature; needing it faster or narrower than that means a real flag system with runtime evaluation, targeting, an audit trail, an owner, and an expiry date. A flag with no removal date is config with extra machinery, and a flag older than one release cycle should be deleted in one direction or the other — **but that rule is about release flags**, whose job ends when the rollout does. Applied to an operational kill switch it instructs you to delete the mitigation: toward on reinstates the incident it exists to stop, toward off removes the feature. That one is permanent by design, and takes a removal *condition* — the guarded call is gone, or its failure mode is structurally impossible — plus a scheduled exercise, because the arm you need at 3am is the arm that never runs. `feature-flagging`'s four types are what decide which of the two you are holding before this rule gets applied to either.
