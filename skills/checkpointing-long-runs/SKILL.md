...
An **obligation** crosses that boundary too, and the rule above is wrong for it in a way worth stating on its own. A prohibition, a stop condition, a "check with me before X" loses its provenance exactly like a claim does — but downgrading one to *assumed* on arrival is not caution, it is the failure itself, because a constraint held at low confidence has already stopped constraining. There is no honest tier for an obligation. It is either in force, in which case it gets re-read from wherever it was written down, or it is gone and the run is now doing something nobody authorized. So obligations are written to the record *before* the work, and every boundary that re-reads the plan re-reads them as well. `bounding-autonomous-work`'s pre-committed stop conditions are the case that matters most here and the likeliest to be quietly summarized into a general sentence about being careful — which reads like the constraint surviving, and is not.
