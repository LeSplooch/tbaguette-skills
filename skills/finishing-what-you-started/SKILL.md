...
description: Use when a task is big enough that stopping short would go unnoticed — a sweep across many files, a long autonomous run, a multi-part request, a multi-sitting build; when a recurring run drops the same clause each pass; when a report is about to say done while part of the request was quietly dropped, sampled, or narrowed; when a summary states counts or coverage from memory rather than from a measurement; when a zero came from an instrument whose host never ran; when work on this keeps coming back unfinished; when the acceptance criteria exist only in your head and the context is getting long; or when a run is about to close while a build, server, watcher, or dispatched agent it started is still running. Also use when a process is about to be left running. Covers writing the acceptance ledger to a file first, watching each check fail before trusting it, surrendering a criterion visibly rather than deleting it, re-measuring every number at report time, and a disposition for every process the run started.
---
...
There is a fourth kind of vacuous pass, and it wears a real number, which is why it gets past people who would never trust a typo'd path. **An instrument that only runs while its host is awake returns a clean zero while the host is asleep** — a layout-shift score read from a browser tab that has never painted, a queue depth from a scheduler that has not ticked, a jank percentage from a window that never drew a frame. The zero is not a measurement of the work. It is the instrument reporting that nothing happened, and nothing happened because nothing ran. Watching the check fail catches it — a surface that cannot be made to fail on an input you have deliberately broken is a surface that did not execute — but the remedy is not to poll more often from outside. Arm the measurement inside the host: have it wait for the host's own wake signal (the visibility event, the first frame, the scheduler's tick), run there, and leave the result somewhere the run can read afterwards. A number produced that way carries its own proof that the host was awake when the reading was taken.
