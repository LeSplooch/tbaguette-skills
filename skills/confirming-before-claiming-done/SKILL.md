...
**The capture is fresh and shows somebody else's surface.** A screenshot or recording taken from a shared instrument — a test phone, a browser profile, a display, a staging box — is the same case in a different medium: it arrived on time, it is real, and it can be a correct picture of whatever owned the surface at that instant, because another session, a dialog, or a hand on the device can take the foreground between two steps of a scripted pass. The positive probe here is the owner: read what had the foreground at capture time, record it beside the capture, and treat a capture without that line as an artifact of unknown source. `formidable`'s audit reference carries the addressing rule — capture by window id, package, or tab, never by whatever has focus.

**The capture predates the change it was taken to show.** A screenshot says what was last composited, not what the layout or the state is, and the compositor keeps its own clock. Taken within a frame or two of a resize, a navigation, or a click, it can hand back the frame from before the reflow: content filling two-thirds of a window whose every element measures full width. It can also be stale *by region*, with the content area already showing the new tab while the tab strip still highlights the old one. When the claim is about geometry or state, measure that property directly (element bounds, the active attribute) and let the picture corroborate. When a picture disagrees with a measurement taken in the same second, suspect the picture first, and capture again once the surface has settled before calling anything a defect.
