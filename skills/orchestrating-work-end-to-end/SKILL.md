...
- any register → `crouton`. It binds what the run *pulls in*, not only what it writes, and that is the half with the money in it: every turn re-sends the conversation, so a file read costs roughly three times its own size. Enough of it to act on without opening that skill — read the range rather than the file, outline anything long before opening it, cap what a command can hand back (`| head`, `--stat`, `-q`), never re-read what is already in context, and say the background once. Read here, because by phase 8 every read has already been paid for.
...
Phase 2's owner column gained a skill in front of it, and the order is the point. Everything in this spine executes the request; nothing in it asks whether the request is the right one, and the frame that decides that arrives in the first message and is never named again. `clairvoyance` is the one move that looks outward instead of forward, and the frame-to-design boundary is where it is nearly free — a reframe costs a paragraph there, the plan at phase 4, and the implementation at phase 6, which is why a brilliant reframe at review is usually declined and should be. It has a seat in every track below, different in each and twice a prohibition. What it produces is sight, never work: every observation routes back to a gate, into the ledger, into the record for `offering-the-next-move`, or to a discard with a reason — and `managing-scope-drift` still governs everything it makes you want to do.

The other tracks reuse this spine rather than replacing it:

- **Diagnose** keeps phases 1 and 3 — a bug still gets a ledger and still gets off the trunk — and runs reproduce → locate → fix → pin in place of phases 2, 4, and 5, under `diagnosing-before-fixing`. It rejoins at phase 6. Its gate before any fix is a reproduction that fails on demand; its gate after is a regression test that failed before the fix and passes after (`regression-test-from-bug`). When the hypothesis list empties or a third fix does not hold, `clairvoyance` runs on the reproduction rather than on the patch: three correct fixes that did not hold are one wrong model applied three times, not three bad patches.
- **Respond** is the one track that does not open with a ledger — there is no request to derive one from. Its equivalent is the **impact statement**: who is affected, how badly, and whether it is growing, written in user terms, and the first gate is that the harm stopped rather than that it is understood. `responding-to-incidents` owns declare → assess → preserve → mitigate → stabilize, and inverts the usual order deliberately: mitigate before you diagnose. The moment users are safe it hands back to **Diagnose**, which picks up the spine normally. Its close-out is `writing-postmortems`, and skipping that because the cause turned out obvious in hindsight is how the same incident returns next quarter. `clairvoyance` has a seat here and it is a late one — never during mitigation, where a frame question is the "let me understand this properly first" red flag exactly, but at stabilize, on what the crude mitigation has now committed you to, and again in the postmortem, on whether an incident of this shape has been here before under another name.
- **Investigate** runs orient → gather → conclude → report. It reaches phase 8 only if it produced an artifact that has to land — a doc, an ADR, a spec. Its close-out gate is `calibrating-confidence`: every claim in the report marked verified, inferred, or assumed, with the unknowns named rather than smoothed over. Before the conclusion is written, `clairvoyance` asks whether the question commissioned is the question that matters — and both get answered, the second marked as inference, because substituting it for the first is not a reframe but a failure to answer.
- **Review** runs orient → cover → judge → deliver, under `handing-off-for-review` and `reviewing-code-deeply`. Its gate is **coverage of the diff**, not confidence in a conclusion: every changed file actually seen, and what is *absent* from the change weighed alongside what is in it — `clairvoyance`'s absence pass is how that second half gets found, since a diff cannot announce the caller nobody updated or the state nobody modelled. It ends at delivery rather than at landing, because the branch belongs to whoever wrote it, and `verifying-review-feedback` governs the return leg when the findings come back argued.
- **Author** runs frame the reader → gather → draft → verify → land. The phase that gets skipped is verify, every time: a document's claims are checked against the thing they describe, the same way code's are, rather than against the memory of it. `writing-durable-docs` decides what will still be true in a year, `explaining-technical-work` sets the altitude, and the specific form — `writing-adrs`, `writing-release-notes`, `writing-postmortems` — owns the shape. `clairvoyance` runs twice here, at *frame the reader* and again before verify: who else reads this, and what this document makes impossible to say later.
- **Change in place** opens with `clairvoyance` and then `deciding-reversibility` — in that order, because reversibility is a question about a chosen operation and asking it first quietly accepts that the operation is the right one, when the cheapest migration is the one that does not happen because the thing can be deleted. A one-way door is then a different piece of work from a two-way one. It runs its own skill from the routing reference, and rejoins at phase 6. An upgrade or a migration is reviewed like any other diff; arriving with no new code of its own is not a reason to skip the seat.
...
Three of those pulls are load-bearing enough to name here. Before phase 1 on any inherited or unfamiliar work, `recovering-agent-context` and `orienting-in-unfamiliar-code` come first — a run framed against a codebase you have not looked at frames the wrong thing. Between phase 1 and phase 2, `clairvoyance`, which is the only pull in the library that questions the request rather than serving it, and the only one whose value collapses if it is deferred. And after phase 7 believes it is finished, `red-teaming-your-own-work` and then `karen-and-the-manager` are this library's standard adversarial close, most warranted exactly when the ordinary review came back clean.
