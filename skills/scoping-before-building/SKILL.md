...
**Sizing a trust circle.** A safety mechanism built on independent corroboration — quorum, multi-sig, k-of-n distinct reporters, "review by two people" — has the population of eligible participants as a hidden design input, and "most secure" and "does anything at all" trade off through that number. Choosing k=3 for "the general public" is a real decision only once the design also says how many members that public actually has today; a private repository with zero other users, a beta with three testers, or a feature nobody has enabled yet can all make "the public" a population of one or zero, and a mechanism requiring three independent corroborators is then inert by construction — silently, until the day it starts working, which is also the day whatever it protects becomes worth attacking. State the population next to the threshold, and say what the mechanism does at population one.
