...
- The most common way this hazard reaches you is a single `pkill -f`/`kill` typed straight into a tool call rather than saved in a script — and the framing "am I writing a script" is exactly why it slips past review: nothing about composing one inline command reads as scripting. If the pattern happens to be a substring of the invoking shell's own command line, the kill takes that shell down mid-call, and the tell is distinctive: every subsequent call in that shell context returns a nonstandard exit code with **no output at all** — not an error message, not a partial result, just silence, because the interpreter died before it could write anything. That symptom looks like a mysterious tool failure and costs several calls to recognize unless you know to read total silence right after a pattern-based kill as the kill having caught its own caller.
...
The quietest form is the same match asked *once*, as a question: `if pgrep -f foo; then echo still running; else restart; fi`. The loop above at least leaves something to find — several processes that never exit. A single query leaves nothing. The shell asking carries the pattern in its own argv, `pgrep` answers yes, the restart is skipped, and the thing under test stays dead while every later step — a capture, a probe, a verification round — runs against a process that is not there. Nothing dies, nothing hangs, no line says anything went wrong, and the false *alive* is indistinguishable from a true one until something downstream fails for a reason nobody connects to a liveness check that passed. In one project this bit three times in a week — twice as a kill, once as this — and the check that passed cost a full verification round against a dead process. Ask liveness by handle, never by pattern: `kill -0 "$pid"` against the saved PID is POSIX and answers for exactly that process. Where no PID was saved, `pgrep -x` matches the kernel's short name only, which an asking shell's argv cannot contain, with the same interpreter caveat `pkill` without `-f` carries above. And the trap does not need a script to spring: an inline `pgrep -f` typed into a tool call is a shell whose argv contains the pattern, whether or not anyone called it a script.
...
| Every call after an inline `pkill -f`/`kill` returns a nonstandard exit code with zero output | that one call killed the shell it ran in; nothing wrote anything because the interpreter died first |
...
| A restart was skipped because the job was "still running"; it was dead | a one-shot `pgrep -f` liveness check matched the asking shell's own command line and answered yes |
...
- A yes-or-no "is it still running" answered by `pgrep -f`, from a shell whose own command line carries the pattern — including a one-liner in a tool call
